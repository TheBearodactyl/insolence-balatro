...
local function unzip_file(zip_path, zip_subpath, out_dir)
    ...
    if is_windows then
        ...
        ok1 = os.execute(string.format(
        'powershell -NoProfile -Command "Expand-Archive -LiteralPath %q -DestinationPath %q -Force"', zip_path, tmp_dir))

        local subfolder = get_subdir(tmp_dir)
        if subfolder then
            local src_path = subpath and tmp_dir .. "\\" .. subfolder .. "\\" .. subpath or tmp_dir .. "\\" .. subfolder
            ok2 = os.execute(string.format('powershell -NoProfile -Command "Move-Item -Path %q -Destination %q -Force"',
                src_path, out_dir))
        else
            ok2 = false
        end
...
local function install_update()
    ...
    local zip_url = string.format("https://github.com/%s/%s/archive/%s.zip", owner, repo, tag)
    local zip_path = ("Mods\\%s-%s.zip"):format(repo, tag)
    local target_dir = ("%s"):format(mod_path)
...
insolence.update_check = function()
