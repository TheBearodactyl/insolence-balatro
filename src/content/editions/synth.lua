...
SMODS.Edition({
	...
	loc_vars = function(self, info_queue, card)
		info_queue[#info_queue + 1] = G.P_CENTERS["e_ins_equalize_ed"]
	...
	calculate = function(self, card, context)
		...
		if context.scoring_hand then
			for i = 1, #context.scoring_hand do
				if context.scoring_hand[i].edition and context.scoring_hand[i].edition.key == "e_ins_equalize_ed" then
		...
		for _, v in pairs(G.jokers.cards) do
			...
			if ed.key and ed.key == "e_ins_equalize_ed" then
				equalize_count = equalize_count + 1
			end
		end
