...
SMODS.Edition({
    ...
    calculate = function(self, card, context)
        ...
        if context.using_consumeable then
            if
                context.consumeable.ability.name == "The Sun"
            ...
            then
                ...
                elseif context.consumeable.ability.name == "The Star" then
                    ...
                    local choices = { "MtF", "FtM" }
                    local rand_idx = math.random(1, 2)
                    msg = choices[rand_idx]
                end
...
function Card:change_suit(new_suit)
    if self.edition and self.edition.key == "e_ins_trans_ed" then
