SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        local chance = function()
            if not libinsolence.is_rigged_cryptid(card) and G.GAME.probabilities.normal then
                return G.GAME.probabilities.normal
            elseif not libinsolence.is_rigged_cryptid(card) then
                return 1
            else
    ...
    calculate = function(self, card, context)
        if context.end_of_round and context.main_eval and not context.blueprint then
            if
                G.GAME.blind.boss
                and not (pseudorandom("Stopped Heart") < G.GAME.probabilities.normal / card.ability.extra.reset_chance)
            then
