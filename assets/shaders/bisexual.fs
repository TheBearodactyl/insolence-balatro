...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	vec3 pink = rgb_to_vec3(214, 2, 112);
	vec3 purple = rgb_to_vec3(155, 79, 150);
	vec3 blue = rgb_to_vec3(0, 56, 168);

	float wave_segment = mod(floor(sin(uv.y) * cos(uv.x) * 10.0 + time * 5.0), 3.0);
	vec3 color;
	...
	vec4 final_color = vec4(color, 1.0) * colour + clamp(bisexual.y, 0.0, 0.001);
	// tex.a = 0.75;
	final_color.a = 0.75;
