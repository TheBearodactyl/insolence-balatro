...
extern MY_HIGHP_OR_MEDIUMP vec2 bugged;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	number high = 0.25 * min(tex.r, min(tex.g, tex.b));
	number low = 4 * max(tex.r, max(tex.g, tex.b));
	number delta = high - low;

	uv = rotate_uv(uv, time / 5);

	number saturation_fac = 1.4; //tan(cos(bugged.x) / sin(bugged.y)) - max(0., 0.05 * (1.1 - delta));

	vec4 hsl = HSL(vec4(tex.r * saturation_fac, tex.g * saturation_fac, tex.b, tex.a));

	float t = bugged.y * 2.221 + mod(time, 1);
	...
	float res = (.5 + .5 * cos((bugged.y) * 2.612 + (field + -0.5) * 3.14));
	hsl.x = hsl.x + 0.005 * length(field_part2);
	hsl.z = sin(hsl.z / 2.5 - res / 4 + sin(bugged.y) / 8 + 0.5) / 1.4;
