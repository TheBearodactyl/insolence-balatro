...
extern MY_HIGHP_OR_MEDIUMP vec2 ace;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	vec3 black = rgb_to_vec3(0, 0, 0);
	vec3 grey = rgb_to_vec3(163, 163, 163);
	vec3 white = rgb_to_vec3(255, 255, 255);
	vec3 purple = rgb_to_vec3(128, 0, 128);

	float wave_segment = mod(floor(sin(uv.y) * cos(uv.x) * 10.0 + time * 5.0), 4.0);
	vec3 color;
	...
	vec4 final_color = vec4(color, 1.0) * colour + clamp(ace.y, 0.0, 0.001);
	// tex.a = 0.75;
	final_color.a = 0.75;
