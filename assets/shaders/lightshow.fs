...
extern MY_HIGHP_OR_MEDIUMP vec2 lightshow;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	vec2 pixel_offset = 4.0 * (-tex.yz + 2.0 * uv.xy) / tex.y;
	float id = 0.5 + 0.5 * cos(time + sin(dot(floor(pixel_offset + 0.5), vec2(113.1, 17.81))) * 43759.545);
	vec3 color_components = 0.5 + 0.5 * cos(time + 2.0 * id + vec3(0.0, 1.0, 2.0));
	vec2 pa = smoothstep(0.0, 0.2, id * (0.5 + 0.5 * cos(6.2831 * pixel_offset)));
	vec4 final_color = vec4(color_components * pa.x * pa.y, 1.0 + clamp(lightshow.x, 0.000001, 0.000001));
