...
extern MY_HIGHP_OR_MEDIUMP vec2 equalize;
...
float round(float x) {
	return (1.0 + sin(x * 7.58765)) * 0.5;
}

vec3 round_vec3(float x) {
	vec3 v;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	float beat = floor(time / BEAT);
	float beat_fract = fract((time / BEAT) + (bar * SUBBEAT));
	float bar_height = sin((round(beat + bar) * (0.5 + clamp(equalize.x, 0.001, 0.001))) + 0.2);
	vec4 col = vec4(round_vec3(bar), 1.0);

	vec4 final_color = vec4(step(uv.y, bar_height * (1.0 - beat_fract * 0.8))) * col;
